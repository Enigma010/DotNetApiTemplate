...
namespace DotNetApiAppCore.Services
{
    ...
    public interface IBaseSingletonService<EntityType, IdType>
    {
        Task<EntityType> RunCommandAsync(string methodName, Func<EntityType, EntityType> changeFunc);
    }
    /// <summary>
    ...
    public class BaseSingletonService<RepositoryType, EntityType, EntityDtoType, IdType>
        : IBaseSingletonService<EntityType, IdType>
        
        where RepositoryType : IGetableSingletonRepository<EntityType, EntityDtoType, IdType>, 
        ...
        where IdType : IComparable
    {
        ...
        public BaseSingletonService(RepositoryType repository, IEventPublisherUnitOfWork eventPublisher, ILogger logger)
        {
        ...
        public async Task<EntityType> RunCommandAsync(string methodName, Func<EntityType, EntityType> command)
        {
            _logger.LogInformationCaller($"{methodName}, commandFunc: {command}", args: [command]);
            using (var unitOfWorks = new UnitOfWorks(_unitOfWorks, _logger))
            {
                Func<Task<EntityType>> retrieveUpdatePublishCmd = async () =>
                {
                    EntityType entity = await _repository.GetAsync();
                    _logger.LogInformation("Running command on {@entity}", entity);
                    command(entity);
                    _logger.LogInformation("Ran command on {@entity}", entity);
